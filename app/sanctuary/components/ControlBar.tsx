    ...
    <footer className="fixed bottom-0 left-0 right-0 z-40 px-6 md:px-10 pb-8 md:pb-12 pointer-events-none flex justify-center">
      <div 
        className={`
          ...
          ${isExpanded 
            ? 'w-full max-w-[1100px] rounded-[3rem] md:rounded-[4rem] p-6 md:p-10 border-white/10' 
        ...
        `}
      >
        <div className={`flex flex-col sm:flex-row items-center gap-6 ${isExpanded ? 'sm:gap-12 w-full' : 'gap-3 md:gap-4'} relative z-10`}>
          ...
          <div className="flex items-center gap-4 md:gap-5 w-full sm:w-auto justify-between sm:justify-start">
            ...
            >
              {isGlobalPlaying 
                ? <Pause className={isExpanded ? "w-6 h-6 md:w-10 md:h-10" : "w-4 h-4 md:w-5 md:h-5"} /> 
                : <Play className={`${isExpanded ? "w-6 h-6 md:w-10 md:h-10 ml-1" : "w-4 h-4 md:w-5 md:h-5 ml-0.5"}`} />
              }
              
              {isGlobalPlaying && (
                <div className={`absolute inset-0 rounded-full bg-moss-200/30 animate-ping opacity-20 ${!isExpanded && 'hidden'}`}></div>
              )}
            </button>
            ...
            {!isExpanded && (
              <div className="flex items-center gap-3 md:gap-4 animate-fade-in mr-auto sm:mr-0 pl-2">
                <div className="flex flex-col">
                  <p className="text-[8px] md:text-[9px] text-moss-500 uppercase font-bold tracking-[0.2em] leading-none mb-1.5">Session</p>
                  <p className="text-lg md:text-xl font-serif italic text-moss-100 leading-none">{activeSounds.length} <span className="text-xs md:text-sm font-sans not-italic text-moss-500 font-normal ml-1">layers</span></p>
                </div>
              </div>
            )}
             ...
             {!isExpanded && (
                 ...
                 className="sm:hidden flex items-center justify-center w-8 h-8 rounded-full bg-white/[0.03] text-moss-400"
               >
          ...
          <button 
            ...
            className={`
              ...
              hidden sm:flex
              ${isExpanded ? 'p-3 self-center sm:self-auto' : 'w-10 h-10 bg-white/[0.03] hover:bg-white/[0.08] ml-2'}
            `}
            title={isExpanded ? "Collapse Controls" : "Expand Controls"}
          >
            {isExpanded ? <ChevronDown className="w-5 h-5" /> : <ChevronUp className="w-4 h-4" />}
          </button>
